...
<body class="bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
    ...
    <!-- Hero -->
    <header class="relative bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-400 text-white py-16">
        <div
            class="absolute inset-0 bg-[url('https://source.unsplash.com/1600x600/?events,conference,people')] bg-cover bg-center opacity-20">
        </div>
        <div class="relative max-w-4xl mx-auto px-6 text-center">
            <h1 class="text-4xl md:text-5xl font-extrabold drop-shadow-lg">Events</h1>
            <p class="mt-4 text-lg text-indigo-100 max-w-2xl mx-auto">
                Browse upcoming and past workshops, seminars, and conferences.
            </p>
        </div>
    </header>
    ...
    <div id="popup" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-xl shadow-lg p-6 w-96 relative">
            <button onclick="closePopup()" class="absolute top-2 right-2 text-gray-500 hover:text-gray-700">✖</button>
            <h2 id="popupTitle" class="text-xl font-bold mb-4">Register</h2>
            <form class="flex flex-col gap-3">
                <input type="text" placeholder="Full Name" class="border rounded-lg px-3 py-2 w-full" required>
                <input type="email" placeholder="Email" class="border rounded-lg px-3 py-2 w-full" required>
                <input type="tel" placeholder="Phone" class="border rounded-lg px-3 py-2 w-full" required>
                <button type="submit"
                    class="bg-indigo-500 text-white py-2 rounded-lg hover:bg-indigo-600">Submit</button>
            </form>
        </div>
    </div>
...
<a href="{{ route('privacy.policy') }}" class="text-gray-400 hover:text-white">
    ...
    <script>
        function openPopup(title) {
            document.getElementById('popupTitle').innerText = "Register for " + title;
        ...
        function filterEvents(type) {
            let upcomingCards = document.querySelectorAll('.event-card.upcoming');
            let pastCards = document.querySelectorAll('.event-card.past');

            if (type === 'upcoming') {
                upcomingCards.forEach(c => c.style.display = 'block');
                pastCards.forEach(c => c.style.display = 'none');
            } else {
                upcomingCards.forEach(c => c.style.display = 'none');
                pastCards.forEach(c => c.style.display = 'block');
            }
        }
